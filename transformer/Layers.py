...
# 2024.1.2 Style Adaptive Layer Norm from stylespeech
...
class FFTBlock_style(nn.Module):
    ''' FFT Block '''
    def __init__(self, d_model, 
                    n_head, d_k, d_v, d_inner, fft_conv1d_kernel_size, style_dim, dropout=0.1):
        super(FFTBlock_style, self).__init__()
...
class PostNet(nn.Module):
    ...
    def forward(self, x):
        dropout = 0.5  # orgin dropout = 0.5
        # dropout = 0.01
        x = x.contiguous().transpose(1, 2)

        for i in range(len(self.convolutions) - 1):
            x = F.dropout(torch.tanh(self.convolutions[i](x)), dropout, self.training)
        x = F.dropout(self.convolutions[-1](x), dropout, self.training)
